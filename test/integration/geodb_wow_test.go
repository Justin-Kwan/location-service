...
func NewSetQuery(Key, member string, lon, lat float64) *redis.GeoQuery {
	return &redis.GeoQuery{
		Coord: &redis.GeoPos{
			Lon: lon,
			Lat: lat,
		},
...
func NewRadiusQuery(Key string, lon, lat, radius float64) *redis.GeoQuery {
	return &redis.GeoQuery{
		Coord: &redis.GeoPos{
			Lon: lon,
			Lat: lat,
...
func NewMoveMemberQuery(member, fromKey, toKey string) *redis.GeoQuery {
	return &redis.GeoQuery{
		Member:  member,
		FromKey: fromKey,
		ToKey:   toKey,
...
func populateGeoDB(t *testing.T) {
	...
	for _, tp := range testPOIs {
		...
		coord, err := geoDB.Get(&redis.GeoQuery{Key: tp.Key, Member: tp.ID})
...
func TestGeoDBSetIfExists_MemberNotFoundCases(t *testing.T) {
	...
	for _, c := range SetIfExists_MemberNotFoundCases {
		...
		_, err = geoDB.Get(&redis.GeoQuery{Key: PrimaryTestKey, Member: c.inputMember})
		assert.EqualError(t, err, c.expectedErr.Error())
	}
}
...
var (
	BatchSetIfExists_NormalCases = []struct {
		inputQueries []*redis.GeoQuery
	}{
		// batch of update queries
		{[]*redis.GeoQuery{
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID1", Coord: &redis.GeoPos{Lon: 67.12421, Lat: 45.87676567}},
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID4", Coord: &redis.GeoPos{Lon: 0, Lat: 0}},
			&redis.GeoQuery{Key: SecondaryTestKey, Member: "test_ID10", Coord: &redis.GeoPos{Lon: 56.51243, Lat: 6.432524}},
		}},
		// batch of update queries
		{[]*redis.GeoQuery{
			&redis.GeoQuery{Key: SecondaryTestKey, Member: "test_ID2", Coord: &redis.GeoPos{Lon: 14, Lat: -75.43}},
			&redis.GeoQuery{Key: SecondaryTestKey, Member: "test_ID8", Coord: &redis.GeoPos{Lon: -100.23, Lat: 5}},
			&redis.GeoQuery{Key: SecondaryTestKey, Member: "test_ID11", Coord: &redis.GeoPos{Lon: 23, Lat: -0.004}},
			&redis.GeoQuery{Key: SecondaryTestKey, Member: "test_ID5", Coord: &redis.GeoPos{Lon: -1.07, Lat: 3}},
			&redis.GeoQuery{Key: SecondaryTestKey, Member: "test_ID6", Coord: &redis.GeoPos{Lon: 1, Lat: -1}},
		}},
		// batch of update queries
		{[]*redis.GeoQuery{
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID12", Coord: &redis.GeoPos{Lon: -97.12421, Lat: 0.0089876}},
		}},
	}
...
func TestBatchSetIfExists_NormalCases(t *testing.T) {
	...
	for _, c := range BatchSetIfExists_NormalCases {
		...
		for _, q := range c.inputQueries {
			coord, err := geoDB.Get(&redis.GeoQuery{Key: q.Key, Member: q.Member})
			if err != nil {
				teardownTests()
				t.Fatalf(err.Error())
			}
...
func TestBatchSetIfExists_EmptyCases(t *testing.T) {
	...
	for _, c := range BatchSetIfExists_NormalCases {
		for _, q := range c.inputQueries {
			_, err := geoDB.Get(&redis.GeoQuery{Key: q.Key, Member: q.Member})
...
func TestDelete_NormalCases(t *testing.T) {
	...
	for _, tp := range testPOIs {
		// function under test
		if err := geoDB.Delete(&redis.GeoQuery{Key: tp.Key, Member: tp.ID}); err != nil {
			teardownTests()
			t.Fatalf(err.Error())
		}

		// assert point of interest is deleted
		_, err := geoDB.Get(&redis.GeoQuery{Key: tp.Key, Member: tp.ID})
		assert.EqualError(t, err, types.ErrMemberNotFound.Error())
	}
}
...
var (
	BatchDelete_NormalCases = []struct {
		...
		{[]*redis.GeoQuery{
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID1"},
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID5"},
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID8"},
		}},
		{[]*redis.GeoQuery{
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID2"},
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID9"},
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID4"},
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID6"},
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID10"},
		}},
		{[]*redis.GeoQuery{
			&redis.GeoQuery{Key: PrimaryTestKey, Member: "test_ID3"},
		}},
...
func TestBatchDelete_NormalCases(t *testing.T) {
	...
	for _, c := range BatchDelete_NormalCases {
		...
		for _, q := range c.inputQueries {
			_, err := geoDB.Get(&redis.GeoQuery{Key: PrimaryTestKey, Member: q.Member})
			assert.EqualError(t, err, types.ErrMemberNotFound.Error())
		}
	}
}
...
func TestBatchDelete_NoQueryCases(t *testing.T) {
	...
	err := geoDB.BatchDelete([]*redis.GeoQuery{}...)
...
func TestMoveMember_NormalCases(t *testing.T) {
	...
	for _, tp := range testPOIs {
		...
		// assert in new index
		coord, err := geoDB.Get(&redis.GeoQuery{Key: oppositeTestKey(tp.Key), Member: tp.ID})
		if err != nil {
			teardownTests()
			t.Fatalf(err.Error())
		}
